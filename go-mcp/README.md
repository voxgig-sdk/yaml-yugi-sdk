...
[MCP](https://modelcontextprotocol.io) server exposing the YamlYugi SDK as
two agent tools — `yaml-yugi_list` and `yaml-yugi_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/yaml-yugi-mcp)
make build

# 2. Provide credentials via the environment
export YAML_YUGI_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user yaml-yugi \
  -- /absolute/path/to/yaml-yugi-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./yaml-yugi-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// yaml-yugi_list: first page of records
{ "entity": "card" }
{ "entity": "card", "query": { } }

// yaml-yugi_load: one record by id
{ "entity": "aggregation", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/yaml-yugi-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user yaml-yugi \
     -- "$PWD"/dist/*/yaml-yugi-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `yaml-yugi_list` and `yaml-yugi_load` tools now appear
   in new sessions. Ask the agent to *"list card using yaml-yugi"*
   and it calls `yaml-yugi_list` with `{"entity":"card"}`.
...
```sh
export YAML_YUGI_APIKEY=sk_live_xxx            # API key
export YAML_YUGI_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `yaml-yugi_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `yaml-yugi_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `YAML_YUGI_APIKEY` | API key sent with every request. |
| `YAML_YUGI_BASE` | Optional override of the API base URL. |
...
```sh
./yaml-yugi-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"yaml-yugi_load","arguments":{"entity":"aggregation","query":{"id":1}}}}'
```
