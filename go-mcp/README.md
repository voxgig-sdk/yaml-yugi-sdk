...
MCP server exposing the YamlYugi SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `yaml-yugi_list` | `entity`, optional `query` map | First page of records as JSON |
| `yaml-yugi_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `yaml-yugi_list` and
`yaml-yugi_load` tools become available in new sessions.
