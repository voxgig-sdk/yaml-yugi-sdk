// Command YamlYugi-mcp is an MCP server exposing the YamlYugi
// SDK as MCP tools.
//
// Two tools are registered:
//
//	YamlYugi_list  — list records of an entity
//	YamlYugi_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: YAML_YUGI_APIKEY carries the API key and
	// YAML_YUGI_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("YAML_YUGI_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("YAML_YUGI_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewYamlYugiSDK(opts)
