...
//     (\`new YamlYugiSDK(...)\` / \`YamlYugiSDK.test(...)\` become
//     \`YamlYugiSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"aggregation":{"test01":{"id":"test01"}},"card":{"test01":{"id":"test01"}},"individual_card":{"test01":{"id":"test01"}},"series":{"test01":{"id":"test01"}},"series_and_archetype":{"test01":{"id":"test01"}},"skill":{"test01":{"id":"test01"}},"skill_card":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new YamlYugiSDK(...)\` and any
// \`YamlYugiSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+YamlYugiSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/YamlYugiSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
