...
describe('IndividualCardEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'individual_card.' + op, live)) return
    }
...
function basicSetup(extra?: any) {
  ...
  let idmap = transform(
    ['individual_card01','individual_card02','individual_card03','card01','card02','card03','rush01','rush02','rush03','tcg_speed_skill01','tcg_speed_skill02','tcg_speed_skill03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
  ...
  const idmapEnvVal = process.env['YAML_YUGI_TEST_INDIVIDUAL_CARD_ENTID']
