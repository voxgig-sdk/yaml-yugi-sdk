...
describe('CardDirect', async () => {
  ...
  test('direct-list-card', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-card', setup.live)) return
    if (skipIfMissingIds(t, setup, ["card01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['YAML_YUGI_TEST_CARD_ENTID']
