...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.YAMLYUGI_TEST_LIVE ||
    'TRUE' === process.env.YAMLYUGI_TEST_OVERRIDE
  ) {
  ...
  m.YAMLYUGI_TEST_EXPLAIN = process.env.YAMLYUGI_TEST_EXPLAIN || m.YAMLYUGI_TEST_EXPLAIN
