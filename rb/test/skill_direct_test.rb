...
class SkillDirectTest < Minitest::Test
  def test_direct_list_skill
    setup = skill_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-skill", setup[:live] ? "live" : "unit")
    if _should_skip
...
def skill_direct_setup(mockres)
