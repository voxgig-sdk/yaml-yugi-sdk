...
class SeriesAndArchetypeEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set YAML_YUGI_TEST_SERIES_AND_ARCHETYPE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    series_and_archetype_ref01_data_dt0_loaded = series_and_archetype_ref01_ent.load(series_and_archetype_ref01_match_dt0, nil)
...
def series_and_archetype_basic_setup(extra)
  ...
  entid_env_raw = ENV["YAML_YUGI_TEST_SERIES_AND_ARCHETYPE_ENTID"]
