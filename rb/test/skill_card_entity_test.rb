...
class SkillCardEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = skill_card_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "skill_card." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set YAML_YUGI_TEST_SKILL_CARD_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    skill_card_ref01_data = nil
    if skill_card_ref01_data_raw.length > 0
...
def skill_card_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "skill_card", "SkillCardTestData.json")
  ...
  entid_env_raw = ENV["YAML_YUGI_TEST_SKILL_CARD_ENTID"]
