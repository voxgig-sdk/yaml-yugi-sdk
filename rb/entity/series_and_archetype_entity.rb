...
class SeriesAndArchetypeEntity
  ...
  def make
    opts = @_entopts.dup
    SeriesAndArchetypeEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = YamlYugiHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [SeriesAndArchetype, Hash] the current SeriesAndArchetype data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = YamlYugiHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of SeriesAndArchetype fields)
  def match_get
  ...
  #
  # @param reqmatch [SeriesAndArchetypeLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.SeriesAndArchetype.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [SeriesAndArchetype, Hash] the loaded SeriesAndArchetype; raises YamlYugiError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = YamlYugiHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
