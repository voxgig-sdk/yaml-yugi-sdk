...
local SkillCardEntity = {}
SkillCardEntity.__index = SkillCardEntity


function SkillCardEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SkillCardEntity)
  self._name = "skill_card"
  self._client = client
...
function SkillCardEntity:get_name()
  return self._name
end


function SkillCardEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SkillCardEntity.new(self._client, opts)
end
...
function SkillCardEntity:mark_deleted()
  self._deleted = true
end


function SkillCardEntity:deleted()
  return true == self._deleted
end


function SkillCardEntity:data_set(args)
  if args ~= nil then
...
function SkillCardEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SkillCardEntity:match_set(args)
  if args ~= nil then
...
function SkillCardEntity:match_get()
...
function SkillCardEntity:stream(action, args, callopts)
...
function SkillCardEntity:_run_op(ctx, post_done)
  local utility = self._utility
