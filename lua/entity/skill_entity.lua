...
function SkillEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SkillEntity)
  self._name = "skill"
  self._client = client
...
function SkillEntity:get_name()
  return self._name
end


function SkillEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SkillEntity.new(self._client, opts)
end
...
function SkillEntity:mark_deleted()
  self._deleted = true
end


function SkillEntity:deleted()
  return true == self._deleted
end


function SkillEntity:data_set(args)
  if args ~= nil then
...
function SkillEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SkillEntity:match_set(args)
  if args ~= nil then
...
function SkillEntity:match_get()
...
function SkillEntity:stream(action, args, callopts)
...
function SkillEntity:_run_op(ctx, post_done)
  local utility = self._utility
