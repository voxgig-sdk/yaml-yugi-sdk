...
local AggregationEntity = {}
AggregationEntity.__index = AggregationEntity


function AggregationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AggregationEntity)
  self._name = "aggregation"
  self._client = client
...
function AggregationEntity:get_name()
  return self._name
end


function AggregationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AggregationEntity.new(self._client, opts)
end
...
function AggregationEntity:mark_deleted()
  self._deleted = true
end


function AggregationEntity:deleted()
  return true == self._deleted
end


function AggregationEntity:data_set(args)
  if args ~= nil then
...
function AggregationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AggregationEntity:match_set(args)
  if args ~= nil then
...
function AggregationEntity:match_get()
...
function AggregationEntity:stream(action, args, callopts)
...
function AggregationEntity:_run_op(ctx, post_done)
  local utility = self._utility
