...
describe("CardEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Card(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Card(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set YAML_YUGI_TEST_CARD_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local card_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.card")))
    local card_ref01_data = nil
    if #card_ref01_data_raw > 0 then
      card_ref01_data = helpers.to_map(card_ref01_data_raw[1][2])
    end

    -- LIST
    local card_ref01_ent = client:Card(nil)
    local card_ref01_match = {}

    local card_ref01_list_result, err = card_ref01_ent:list(card_ref01_match, nil)
    assert.is_nil(err)
...
function card_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("YAML_YUGI_TEST_CARD_ENTID")
