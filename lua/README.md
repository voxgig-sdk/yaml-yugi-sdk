...
The Lua SDK for the YamlYugi API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Aggregation(nil):load({ id = "example_id" }, nil)
...
local result, err = client:YamlYugi(nil):load(
  { id = "test01" }, nil
)
