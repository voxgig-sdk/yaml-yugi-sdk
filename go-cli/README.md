...
boru-driven command-line client **and** interactive REPL for the YamlYugi
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/yaml-yugi-cli)
make build
...
./yaml-yugi-cli load 1 aggregation            # {id:1} shorthand
./yaml-yugi-cli load '{id:1}' aggregation       # explicit match map

# 5. Override the API base URL for a single call
YAML_YUGI_BASE=https://api.example.com ./yaml-yugi-cli load 1 aggregation

# 6. No arguments -> interactive REPL
./yaml-yugi-cli
yaml-yugi> load 1 aggregation
yaml-yugi> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/yaml-yugi-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/yaml-yugi-cli load 1 aggregation
   ```
...
```sh
./yaml-yugi-cli load 1 aggregation          # scalar shorthand for {id:1}
./yaml-yugi-cli load '{id:1}' aggregation     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `yaml-yugi>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `aggregation`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/yaml-yugi-cli`. |
...
(`./yaml-yugi-cli <expr>`) and interactively (the REPL), and expressions compose the
