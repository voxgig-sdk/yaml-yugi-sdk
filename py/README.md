...
The Python SDK for the YamlYugi API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Aggregation(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.YamlYugi(None).load(
    {"id": "test01"}, None
)
