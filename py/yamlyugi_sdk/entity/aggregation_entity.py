...
class AggregationEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return AggregationEntity(self._client, opts)
    ...
    def match_get(self) -> Aggregation:
    ...
    def load(self, reqmatch=None, ctrl=None) -> Aggregation:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Aggregation().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
