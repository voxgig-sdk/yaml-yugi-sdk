    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set YAML_YUGI_TEST_SKILL_CARD_ENTID JSON to run live")
        client = setup["client"]
        ...
        skill_card_ref01_data_dt0_loaded = skill_card_ref01_ent.load(skill_card_ref01_match_dt0, None)
...
def _skill_card_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("YAML_YUGI_TEST_SKILL_CARD_ENTID"))
