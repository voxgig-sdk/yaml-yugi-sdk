...
class TestCardEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Card(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Card(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set YAML_YUGI_TEST_CARD_ENTID JSON to run live")
        client = setup["client"]
...
def _card_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("YAML_YUGI_TEST_CARD_ENTID"))
