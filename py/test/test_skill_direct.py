...
class TestSkillDirect:
    ...
    def test_should_direct_list_skill(self):
        setup = _skill_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-skill", "live" if setup["live"] else "unit")
        if _skip:
...
def _skill_direct_setup(mockres):
