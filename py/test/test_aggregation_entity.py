    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set YAMLYUGI_TEST_AGGREGATION_ENTID JSON to run live")
        client = setup["client"]
        ...
        aggregation_ref01_data_dt0_loaded, err = aggregation_ref01_ent.load(aggregation_ref01_match_dt0, None)
        assert err is None
...
def _aggregation_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("YAMLYUGI_TEST_AGGREGATION_ENTID"))
