...
class TestSeriesAndArchetypeEntity:

    def test_should_create_instance(self):
        testsdk = YamlYugiSDK.test(None, None)
        ent = testsdk.SeriesAndArchetype(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _series_and_archetype_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "series_and_archetype." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set YAML_YUGI_TEST_SERIES_AND_ARCHETYPE_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        series_and_archetype_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.series_and_archetype")))
        series_and_archetype_ref01_data = None
        if len(series_and_archetype_ref01_data_raw) > 0:
...
def _series_and_archetype_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/series_and_archetype/SeriesAndArchetypeTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("YAML_YUGI_TEST_SERIES_AND_ARCHETYPE_ENTID"))
