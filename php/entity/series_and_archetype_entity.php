...
class SeriesAndArchetypeEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "series_and_archetype";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new SeriesAndArchetypeEntity($this->_client, $opts);
    }

    /**
     * @param SeriesAndArchetype|array $args SeriesAndArchetype data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = YamlYugiHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return SeriesAndArchetype|array The current SeriesAndArchetype data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of SeriesAndArchetype fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = YamlYugiHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of SeriesAndArchetype fields).
     */
    ...
    /**
     * Load a single SeriesAndArchetype.
     *
     * @param SeriesAndArchetypeLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed SeriesAndArchetypeLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return SeriesAndArchetype|array The loaded SeriesAndArchetype as an assoc-array at the
     *   SDK boundary; throws YamlYugiError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = YamlYugiHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
