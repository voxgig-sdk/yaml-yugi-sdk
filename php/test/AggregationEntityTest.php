...
class AggregationEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set YAML_YUGI_TEST_AGGREGATION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function aggregation_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("YAML_YUGI_TEST_AGGREGATION_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["YAML_YUGI_TEST_AGGREGATION_ENTID"]);
