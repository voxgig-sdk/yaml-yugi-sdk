...
class IndividualCardEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set YAML_YUGI_TEST_INDIVIDUAL_CARD_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function individual_card_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["individual_card01", "individual_card02", "individual_card03", "yugipedia01"] as $k) {
    ...
    $entid_env_raw = getenv("YAML_YUGI_TEST_INDIVIDUAL_CARD_ENTID");
