...
class CardEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set YAML_YUGI_TEST_CARD_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $card_ref01_data = null;
        if (count($card_ref01_data_raw) > 0) {
...
function card_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("YAML_YUGI_TEST_CARD_ENTID");
