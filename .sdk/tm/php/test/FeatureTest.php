...
class FtCtrl extends YamlYugiControl
{
...
class FtClient
{
    ...
    private YamlYugiUtility $utility;

    public function __construct(YamlYugiUtility $utility, array $options)
    {
    ...
    public function get_utility(): YamlYugiUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (YamlYugiContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public YamlYugiContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (YamlYugiContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = YamlYugiConfig::make_feature($name);
    ...
    private static function buildUrl(YamlYugiSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof YamlYugiError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new YamlYugiResponse($fetched);
        }
    ...
    private function fail(YamlYugiContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (YamlYugiContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
