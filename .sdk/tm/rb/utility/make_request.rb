...
module YamlYugiUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = YamlYugiResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = YamlYugiResponse.new(fetched)
    else
