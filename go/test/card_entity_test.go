...
func TestCardEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set YAML_YUGI_TEST_CARD_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		cardRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.card", setup.data)))
		var cardRef01Data map[string]any
		if len(cardRef01DataRaw) > 0 {
			cardRef01Data = core.ToMapAny(cardRef01DataRaw[0][1])
		}
		...
		_, cardRef01ListOk := cardRef01ListResult.([]any)
		if !cardRef01ListOk {
...
func cardBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"card01", "card02", "card03", "rush01", "rush02", "rush03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("YAML_YUGI_TEST_CARD_ENTID")
	...
	idmapResolved := core.ToMapAny(env["YAML_YUGI_TEST_CARD_ENTID"])
