...
import (
	...
	omni "github.com/voxgig-sdk/yaml-yugi-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.YamlYugiError); is {
				out := map[string]any{
					"name":    "YamlYugiError",
...
func omniCtx(arg any, client *sdk.YamlYugiSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.YamlYugiSDK); is {
			client = live
