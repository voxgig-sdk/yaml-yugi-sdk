...
func TestSkillCardEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		skillCardRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.skill_card")))
...
func skill_cardBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"skill_card01", "skill_card02", "skill_card03", "yugipedia01"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("YAML_YUGI_TEST_SKILL_CARD_ENTID")
	...
	idmapResolved := core.ToMapAny(env["YAML_YUGI_TEST_SKILL_CARD_ENTID"])
